...
public class CommandDelete implements ISubCommand {
    ...
    public void handleCommand(EntityPlayer player, String[] args) {
        if (args.length < 2 || args.length > 2) {
            player.sendMessage(new TextComponentString("Usage: /tbuilder delete <name> [Deletes the specified schematic]"));
            return;
        }
        else {
            if (SchematicHandler.getFile(args[1]) != null) {
                SchematicHandler.deleteCompoundFile(args[1]);
                player.sendMessage(new TextComponentString(args[1] + " Deleted"));
            }
            else player.sendMessage(new TextComponentString(args[1] + " Dose not exist"));
        }
    }
    ...
    public String[] helpInfo(EntityPlayer sender) {
        return new String[]{"Usage: /tt-schematic delete <name>", "", "Deletes the specified schematic"};
    }
}
