...
public class CommandPaste implements ISubCommand {
    ...
    public void handleCommand(EntityPlayer player, String[] args) {
        if (args.length < 2 || args.length > 3) {
            player.sendMessage(new TextComponentString("/tbuilder paste <name> {-i} [Past the specified schematic at your coords. -i will make it ignore air]"));
            return;
        }
        else {
            if (SchematicHandler.getFile(args[1]) == null) {
                player.sendMessage(new TextComponentString(args[1] + " Dose not exist"));
                return;
            }
            else {
                boolean ia = args.length == 3 && args[2].equals("-i");
                try {
                    SchematicHandler.loadAreaFromCompound(SchematicHandler.loadCompoundFromFile(args[1]), player.world, (int) Math.floor(player.posX), (int) Math.floor(player.posY), (int) Math.floor(player.posZ), !ia);
                }
                catch (SchematicHandler.SchematicException e) {
                    e.printStackTrace();
                }
                player.sendMessage(new TextComponentString(args[1] + " Successfully added to world"));
            }
        }
    }
