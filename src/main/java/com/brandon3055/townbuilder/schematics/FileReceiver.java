...
public class FileReceiver {
    ...
    public void receiveFile(String fileName, NetHandlerPlayServer netHandler) {
        if (this.thread != null && this.thread.getTransferInProgress()) {
            netHandler.player.sendMessage(new TextComponentString(TextFormatting.RED + "A file transfer is already in progress from another client"));
            return;
        }
        this.client = netHandler.player;
        this.clientAddress = netHandler.netManager.channel().remoteAddress();
        LogHelper.info("Receiving file from: " + this.clientAddress);
        client.sendMessage(new TextComponentString("Receiving file from: " + this.clientAddress));
    ...
    public boolean getTransferInProgress() {
        return thread != null && thread.getTransferInProgress();
    }
    ...
    public class ReceiverThread extends Thread {
        ...
        public void run() {
            try {
                ...
                client.sendMessage(new TextComponentString("Attempting to connect"));
                ...
                client.sendMessage(new TextComponentString("Connection established"));
                ...
                receiver.client.sendMessage(new TextComponentString(TextFormatting.GREEN + "Upload in progress..."));
                ...
                receiver.client.sendMessage(new TextComponentString(TextFormatting.GREEN + "Upload Complete"));
            }
            catch (IOException e) {
                client.sendMessage(new TextComponentString(TextFormatting.RED + "Upload Failed [IOException]"));
                e.printStackTrace();
            }
